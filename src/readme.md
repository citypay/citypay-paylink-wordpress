...
If no postback url is specified, a postback url `https://your-website/your-payment-page/?CPPP_=postback` will be set. The plugin will
...
the plugin function `CPPP_template_redirect_dispatcher()`. This can be done in the template `functions.php` file.
...
add_action('template_redirect', 'overridden_CPPP_template_redirect_dispatcher');

function overridden_CPPP_template_redirect_dispatcher() {
    if (isset($_GET[CPPP_DISPATCHER])) {
        $action = $_GET[CPPP_DISPATCHER];
        switch ($action) {
            case 'pay':
                CPPP_make_payment();
                break;
            ...
            case 'postback':
                overridden_CPPP_template_redirect_on_postback(); // <---- add your override logic 
                break;
